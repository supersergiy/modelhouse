...
def coarsen_mask(mask, n=1, flip=True):
    ...
    for _ in range(n):
        if isinstance(mask, np.ndarray):
            ...
            mask = mask.float()
            kernel_var = torch.FloatTensor(kernel).to(mask.device).unsqueeze(0).unsqueeze(0)
            k = torch.nn.Parameter(data=kernel_var, requires_grad=False)
            if flip:
                mask = 1 - mask
            mask =  (torch.nn.functional.conv2d(mask.unsqueeze(1),
                kernel_var, padding=1) > 1).squeeze(1).float()
            if flip:
                mask = 1 - mask
    return mask
...
def get_mse_and_smoothness_masks(bundle,
    ...
    binarized_src_defects = (bundle['src_defects'] > 0.01).float()
    src_defects = (1 - binarized_src_defects)
    tgt_defects = (1 - bundle['tgt_defects']).float()
    ...
    warped_src_defects_mse = ((warped_src_defects_mse) > mask_warp_threshold).float()
    ...
    warped_src_defects_posmse = ((warped_src_defects_posmse) > mask_warp_threshold).float()
    ...
    warped_defects_smoo = ((warped_defects_smoo) > mask_warp_threshold).float()

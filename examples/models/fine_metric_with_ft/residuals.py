...
def res_warp_img(img, res_in, is_pix_res=True, padding_mode='zeros', mode="bilinear",
    ...
    while len(img.shape) < 4:
        img = img.unsqueeze(0)
    while len(res_in.shape) < 4:
        res_in = res_in.unsqueeze(0)
    if inv:
        res = res_in.permute(0, 3, 1, 2).field()
...
def res_warp_img_old(img, res_in, is_pix_res=True, padding_mode='zeros', mode="bilinear",
        inv=False):
    if inv:
        print ("inv")
    else:
...
def combine_residuals(a, b, is_pix_res=True, inv=False):
    if inv:
        original_shape = a.shape
        while len(a.shape) < 4:
            a = a.unsqueeze(0)
            b = b.unsqueeze(0)
        a = ~(a.permute(0, 3, 1, 2).field().from_pixels())
        b = ~(b.permute(0, 3, 1, 2).field().from_pixels())

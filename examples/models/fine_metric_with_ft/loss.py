...
def similarity_sampling_loss(sample_size, unsup_loss, sample_coverage=0.001, min_nonblack=0.5):
    ...
    def loss_fn(bundle):
        mse_mask = unsup_loss(bundle)['mse_mask']
...
def multilevel_metric_loss(levels, mip_in, loss_fn,
    ...
    def compute_loss(loss_bundle, crop=32):
        ...
        for l in levels:
            ...
            for _ in range(l - mip_in):
                for k in loss_bundle.keys():
                    if 'field' in k or 'res' in k:
                        loss_bundle_emb[k] = residuals.downsample_residuals(loss_bundle_emb[k])
                    elif 'src_' in k or 'tgt_' in k:
                        loss_bundle_emb[k] = torch.nn.functional.max_pool2d(loss_bundle_emb[k], 2)
            if norm_embeddings:
                with torch.set_grad_enabled(True):
                    #normer = augment.SergiyNorm(per_feature=True, bad_fill=0, filter_defects=True)
                    #loss_bundle_emb = normer(loss_bundle_emb)
                    loss_bundle_emb = augment.normalize_bundle(loss_bundle_emb, per_feature_var=True, mask_fill=0)

            '''full_weight = torch.ones_like(embedding[:, 0:1], device=embedding.device)
            half_weight = torch.ones_like(embedding[:, 0:1], device=embedding.device) * 2

            weight = torch.cat(([half_weight] + [full_weight] * 3), 1)
            loss_bundle_emb['src'] = weight * loss_bundle_emb['src']
            loss_bundle_emb['tgt'] = weight * loss_bundle_emb['tgt']'''

            loss_bundle_emb['pred_tgt'] = loss_bundle_emb['src']
            if 'src_field' in loss_bundle_emb:
                loss_bundle_emb['pred_tgt'] = res_warp_img(
                        loss_bundle_emb['src'],
                        loss_bundle_emb['src_field'])

                #loss_bundle_emb['src_defects'] = res_warp_img(
                #        loss_bundle_emb['src_defects'],
                #        loss_bundle_emb['src_field'])
            ...
            loss_bundle_emb['pred_tgt'] = res_warp_img(loss_bundle_emb['src'],
                                                       loss_bundle_emb['pred_res'],
                                                       is_pix_res=True)
